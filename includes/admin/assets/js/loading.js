	...
	ProgressButton.prototype._stop = function( status ) {
		...
		setTimeout( function() {
			...
			if( typeof status === 'number' ) {
				...
				// Trigger an event so we can change the text
				jQuery( self.button ).trigger( 'studiorum_loading_callback_after_state_class', [status,self.button] );
	...
	[].slice.call( document.querySelectorAll( 'button.progress-button' ) ).forEach( function( bttn ) {
		new ProgressButton( bttn, {
			callback : function( instance ) {
				jQuery( bttn ).trigger( 'studiorum_loading_callback_start', [bttn,instance] );
			}
		} );
	} );
